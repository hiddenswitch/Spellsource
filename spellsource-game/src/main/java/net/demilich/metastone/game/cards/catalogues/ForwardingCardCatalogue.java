...
public final class ForwardingCardCatalogue implements CardCatalogue {

	private final List<CardCatalogue> cardCatalogues;

	public ForwardingCardCatalogue(List<CardCatalogue> cardCatalogues) {
	...
	public Map<String, DeckFormat> formats() {
		return cardCatalogues.stream().flatMap(cc -> cc.formats().entrySet().stream()).collect(Collectors.toMap(Map.Entry::getKey, Map.Entry::getValue));
	}
	...
	public @NotNull Map<String, Card> getCards() {
		return cardCatalogues.stream().flatMap(cc -> cc.getCards().entrySet().stream()).collect(Collectors.toMap(Map.Entry::getKey, Map.Entry::getValue));
	}

	@Override
	public Card getCardById(@NotNull String id) {
		for (var cc : cardCatalogues) {
			var card = cc.getCardById(id);
			if (card != null) {
				return card;
			}
		}
	...
	public @NotNull Map<String, CardCatalogueRecord> getRecords() {
		return cardCatalogues.stream().flatMap(cc -> cc.getRecords().entrySet().stream()).collect(Collectors.toMap(Map.Entry::getKey, Map.Entry::getValue));
	}
