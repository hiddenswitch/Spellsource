...
public class ListCardCatalogue implements CardCatalogue {
	...
	protected final Map<String, DeckFormat> formatsByName = new ConcurrentHashMap<>(64);
	protected final Set<String> bannedCardIds = Sets.newConcurrentHashSet();
	protected final Set<String> hardRemovalCardIds = Sets.newConcurrentHashSet();
	protected final Map<String, Card> cards = new ConcurrentHashMap<>(8196);
	protected final Map<String, CardCatalogueRecord> records = new ConcurrentHashMap<>(8196);
	protected final Multimap<String, CardCatalogueRecord> recordsByName = Multimaps.newSetMultimap(new HashMap<>(8196), Sets::newConcurrentHashSet);
	protected final Map<String, Card> classCards = new ConcurrentHashMap<>(64);
	protected final Map<String, Card> heroCards = new ConcurrentHashMap<>(64);
	protected final Map<String, Card> formatCardsByName = new ConcurrentHashMap<>(64);
	protected final Multimap<DeckFormat, Card> classCardsForFormat = Multimaps.newSetMultimap(new ConcurrentHashMap<>(64), Sets::newConcurrentHashSet);
	protected final Multimap<DeckFormat, String> baseClassesForFormat = Multimaps.newSetMultimap(new ConcurrentHashMap<>(64), Sets::newConcurrentHashSet);
	...
	protected void updatedWith(Map<String, CardDesc> cardDescs) {
		...
		var formatCards = newCards.stream()
				.filter(card -> card.getCardType() == Spellsource.CardTypeMessage.CardType.FORMAT)
				.toList();

		formatsByName.put(FORMAT_NAME_ALL, this.all);
		for (var formatCard : formatCards) {
			formatsByName.put(formatCard.getName(), new DeckFormat()
		...
		classCards.putAll(newCards.stream()
				.filter(c -> c.getCardType() == Spellsource.CardTypeMessage.CardType.CLASS)
		...
		heroCards.putAll(
				classCards.values().stream().map(value -> getCardById(Objects.requireNonNull(value).getHero())).collect(toMap(Card::getHeroClass, Function.identity())));
	}
