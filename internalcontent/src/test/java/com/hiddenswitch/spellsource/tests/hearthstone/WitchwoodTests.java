...
public class WitchwoodTests extends TestBase {
	...
	public void testTessGreymane() {
		runGym((context, player, opponent) -> {
			...
			assertTrue(opponent.getHero().hasAttribute(Attribute.RESERVED_BOOLEAN_3));
			...
			Mockito.doAnswer(invocation -> {
				assertTrue(willReplay.remove(((Card) invocation.getArgument(1)).getCardId()));
				return invocation.callRealMethod();
			}).when(spyLogic).revealCard(any(), any());
			playMinionCard(context, player, "minion_tess_greymane");
			assertEquals(willReplay.size(), 0);
			assertFalse(battlecryTarget.hasAttribute(Attribute.RESERVED_BOOLEAN_2), "The battlecries should not have been resolved.");
			assertFalse(context.getEntities().anyMatch(e -> e.hasAttribute(Attribute.RESERVED_BOOLEAN_3)), "There should have never been a valid target for the spell that adds this attribute.");
		}, "BLUE", "BLUE");
	}
