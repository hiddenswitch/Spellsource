...
public class TheGrandTournamentTests extends TestBase {
	...
	public void testSaboteur() {
		runGym((context, player, opponent) -> {
			...
			assertEquals(costOf(context, opponent, heroPower), 2);
			context.endTurn();
			assertEquals(costOf(context, opponent, heroPower), 7);
			context.endTurn();
			context.endTurn();
			assertEquals(costOf(context, opponent, heroPower), 2);
		});
	...
	public void testKnightOfTheWild() {
		runGym((context, player, opponent) -> {
			...
			playCard(context, player, "minion_bloodfen_raptor");
			playCard(context, player, "minion_bloodfen_raptor");
			assertEquals(costOf(context, player, knight), knight.getBaseManaCost() - 2);
		});
	}
	...
	public void testSideshowSpelleater() {
		runGym((context, player, opponent) -> {
			...
			assertEquals(player.getHero().getHeroPower().getHeroClass(), opponent.getHero().getHeroPower().getHeroClass());
		}, "BLACK", "VIOLET");
	}
	...
	public void testKingsDefenderAttendee() {
		runGym((context, player, opponent) -> {
			...
			assertEquals(hero.getWeapon().getDurability(), 3);
