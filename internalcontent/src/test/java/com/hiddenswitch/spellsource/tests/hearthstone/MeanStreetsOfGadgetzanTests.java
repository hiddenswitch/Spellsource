...
public class MeanStreetsOfGadgetzanTests extends TestBase {
	...
	public void testMayorNoggenfogger() {
		...
		runGym((context, player, opponent) -> {
			...
			Mockito.doAnswer(invocation -> {
				...
				assertEquals(targets.size(), 4, "Noggen, Footman, Worgen and the hero are all the valid targets");
				List<Integer> ids = targets.stream().map(Entity::getReference).map(EntityReference::getId).collect(Collectors.toList());
				assertTrue(ids.containsAll(Stream.of(noggen, footman, stealth, player.getHero()).map(Entity::getReference).map(EntityReference::getId).collect(Collectors.toList())));
				latch.countDown();
		...
		runGym((context, player, opponent) -> {
			...
			Mockito.doAnswer(invocation -> {
				List<Entity> targets = invocation.getArgument(0);
				assertEquals(targets.size(), 2, "Pirates are the only valid targets");
				List<Integer> ids = targets.stream().map(Entity::getReference).map(EntityReference::getId).collect(Collectors.toList());
				assertTrue(ids.containsAll(Stream.of(pirate1, pirate2).map(Entity::getReference).map(EntityReference::getId).collect(Collectors.toList())));
				latch.countDown();
				return invocation.callRealMethod();
			}).when(spyLogic).getRandom(Mockito.anyList());
	...
	public void testVirmenSensei() {
		runGym((context, player, opponent) -> {
			...
			overrideBattlecry(context, player, battlecryActions -> {
				...
				assertEquals(battlecryActions.get(0).getTargetReference(), beast.getReference());
				latch.countDown();
	...
	public void testWrathion() {
		...
		factory.run((context, player, opponent) -> {
			...
			assertEquals(player.getHero().getHp(), preFatigue - 1, "Wrathion should successfully deal fatigue.");
		});
		...
		factory.run((context, player, opponent) -> {
			...
			playCard(context, player, "minion_wrathion");
			assertEquals(player.getHero().getHp(), hp, "Wrathion should not have dealt fatigue.");
		});
		...
		factory.run((context, player, opponent) -> {
			...
			playCard(context, player, "minion_wrathion");
			assertEquals(player.getHero().getHp(), hp - 1, "Wrathion should not have dealt fatigue.");
		});
	}
	...
	public void testLunarVisions() {
		runGym((context, player, opponent) -> {
			...
			assertTrue(player.getHand().stream().allMatch(card -> costOf(context, player, card)
	...
	public void testShakuTheCollector() {
		runGym((context, player, opponent) -> {
			...
			assertTrue(player.getHand().get(0).hasHeroClass(opponent.getHero().getHeroClass()));
	...
	public void testInkmasterSolia() {
		runGym((context, player, opponent) -> {
			...
			assertEquals(costOf(context, player, fireball), fireball.getBaseManaCost());
			playCard(context, player, "minion_inkmaster_solia");
			assertEquals(costOf(context, player, fireball), 0);
			context.endTurn();
			assertEquals(costOf(context, player, fireball), fireball.getBaseManaCost());
		});
	}
