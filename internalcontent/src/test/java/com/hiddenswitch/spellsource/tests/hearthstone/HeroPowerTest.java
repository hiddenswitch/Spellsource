...
public class HeroPowerTest extends TestBase {
	...
	public void testArmorUp() {
		runGym((context, player, opponent) -> {
			...
			assertEquals(hero.getHp(), GameLogic.MAX_HERO_HP);
			...
			int armorUpBonus = 2;
			assertEquals(hero.getHp(), GameLogic.MAX_HERO_HP);
			assertEquals(hero.getArmor(), armorUpBonus);
			...
			assertEquals(hero.getHp(), GameLogic.MAX_HERO_HP - armorUpBonus);
			assertEquals(hero.getArmor(), 0);
			...
			assertEquals(hero.getHp(), GameLogic.MAX_HERO_HP - armorUpBonus);
			assertEquals(hero.getArmor(), armorUpBonus / 2);
		}, "RED", "RED");
	...
	public void testFireblast() {
		runGym((context, player, opponent) -> {
			...
			assertEquals(victim.getHp(), victimStartHp - fireblastDamage);
		}, "BLUE", "RED");
	}
	...
	public void testLesserHeal() {
		runGym((context, player, opponent) -> {
			...
			assertEquals(priest.getHp(), GameLogic.MAX_HERO_HP - lesserHealing);
			...
			assertEquals(priest.getHp(), GameLogic.MAX_HERO_HP);
			context.performAction(context.getPlayer1().getId(), lesserHeal);
			assertEquals(priest.getHp(), GameLogic.MAX_HERO_HP);
		}, "WHITE", "RED");
	...
	public void testLifeTap() {
		runGym((context, player, opponent) -> {
			...
			assertEquals(warlock.getHp(), GameLogic.MAX_HERO_HP);
			...
			assertEquals(warlock.getHp(), GameLogic.MAX_HERO_HP - lifeTapDamage);
