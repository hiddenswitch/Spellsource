...
public class BoomsdayProjectTests extends TestBase {
    ...
    @Test
    public void testUnexptedReults() { //well hopefully we can get some expected results here
        runGym((context, player, opponent) -> {
    ...
    public void testAcademicEspionage() {
        ...
        runGym((context, player, opponent) -> {
            ...
            for (Card card : player.getDeck()) {
                assertEquals(context.getLogic().getModifiedManaCost(player, card), 1, card.getName() + " mana");
    ...
    @Test
    public void testCommanderMorrigan() {
        runGym((context, player, opponent) -> {
            Minion morrigan = playMinionCard(context, player, "minion_commander_morrigan");
            ...
            assertEquals(player.getDeck().get(0).getName(), "Commander Morrigan", "Commander Morrigan");
            context.endTurn();
            context.endTurn();
            playCard(context, player, player.getHand().get(0));
            context.endTurn();
        ...
        runGym((context, player, opponent) -> {
            shuffleToDeck(context, player, "minion_commander_morrigan");
            playCard(context, player, "minion_spiritsinger_umbra");
            playCard(context, player, "minion_knife_juggler");
            playCard(context, player, "minion_commander_morrigan");
            assertTrue(opponent.getHero().isDestroyed());
        });
