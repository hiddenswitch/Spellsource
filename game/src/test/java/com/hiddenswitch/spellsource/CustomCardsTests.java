...
public class CustomCardsTests extends TestBase {
	...
	public void testEndOfTheLineSapInteraction() {
		...
		runGym((context, player, opponent) -> {
			...
			context.endTurn();
			playCard(context, player, "spell_sap", beast);
			context.endTurn();
	...
	public void testArmageddonVanguard() {
		runGym((context, player, opponent) -> {
			...
			context.endTurn();
			int opponentHp = opponent.getHero().getHp();
			playCard(context, opponent, "spell_razorpetal", beast);
	...
	public void testFifiFizzlewarp() {
		...
		runGym((context, player, opponent) -> {
			...
			// Cost 2 card
			Card shouldNotBeDrawn = putOnTopOfDeck(context, player, "minion_test_3_2");
		...
		runGym((context, player, opponent) -> {
			...
			playCard(context, player, "spell_test_deal_6", tirion);
			playCard(context, player, "spell_test_deal_6", tirion);
	...
	public void testBlackLotus() {
		...
		runGym((context, player, opponent) -> {
			...
			context.endTurn();
			playCard(context, player, "spell_razorpetal", beast);
	...
	public void testFleetfootedScout() {
		runGym((context, player, opponent) -> {
			...
			playCard(context, player, "spell_test_deal_6", fleetfooted);
  ...
  @Test
	...
	public void testBulletWithYourName() {
		runGym((context, player, opponent) -> {
			...
			assertTrue(wisp.hasAttribute(Attribute.QUICK_DRAW));
	...
	public void testCrateOfDynamite() {
		runGym((context, player, opponent) -> {
			Minion wisp = playMinionCard(context, player, "minion_wisp");
			playCard(context, player, "spell_crate_of_dynamite", wisp);
	...
	public void testHiredGunsmith() {
		runGym((context, player, opponent) -> {
			Card weapon1 = receiveCard(context, player, "weapon_fiery_war_axe");
			Card nonWeapon = receiveCard(context, player, "minion_bloodfen_raptor");
			...
			Card weapon2 = receiveCard(context, player, "weapon_assassins_blade");
	...
	public void testPlanAhead() {
		runGym((context, player, opponent) -> {
			Stream.of("minion_wisp", "minion_murloc_raider", "minion_bloodfen_raptor", "minion_river_crocolisk",
				"minion_magma_rager", "minion_chillwind_yeti", "minion_oasis_snapjaw", "minion_boulderfist_ogre",
					"minion_war_golem", "minion_core_hound").forEach(s -> shuffleToDeck(context, player, s));
	...
	public void testMassBetrayal() {
		runGym((context, player, opponent) -> {
			...
			for (int i = 0; i < 6; i++) {
				shuffleToDeck(context, opponent, "minion_wisp");
				Minion wisp = playMinionCard(context, opponent, "minion_wisp");
				playCard(context, opponent, "spell_trigger_happy", wisp);
			}
	...
	public void testCaptainKarver() {
		runGym((context, player, opponent) -> {
			playCard(context, player, "weapon_fiery_war_axe");
