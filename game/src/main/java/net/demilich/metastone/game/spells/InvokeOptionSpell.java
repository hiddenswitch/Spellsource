...
public class InvokeOptionSpell extends Spell {
    ...
    protected void onCast(GameContext context, Player player, SpellDesc desc, Entity source, Entity target) {
        ...
        if (mana > 0) {
            ...
            context.fireGameEvent(new InvokedEvent(context, player.getId(), source.getSourceCard(), mana));
        }
    }
    ...
    public static Card getTempCard(GameContext context, SpellDesc invokeOptionSpellDesc, Card sourceCard) {
        ...
        String description = invokeOptionSpellDesc.getString(SpellArg.DESCRIPTION);
        int mana = invokeOptionSpellDesc.getInt(SpellArg.MANA, 0);
