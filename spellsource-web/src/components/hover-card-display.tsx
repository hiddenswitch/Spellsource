...
export const useHoverCardDisplay = (options?: {translateX?: string, translateY?: string}) => {
  ...
  const HoverCardDisplay = () => {
    return card ? <div className={styles.hoverCardDisplay}
                style={{display: card ? "block" : "none", left: `${docX}px`, top: `${docY}px`, transform: `translateX(${translateX}) translateY(${translateY})`}}>
      <CardDisplay {...card}/>
    </div> : <></>
  }
