...
export class RgbColour {
  ...
  /** The alpha component of the colour, ranging from 0 to 1. */
  a: number;
   ...
   * @param a The initial amount of alpha. Defaults to 1.
   */
  ...
  /**
   * Returns a hexadecimal string in the format #rrggbbaa representing the colour.
  ...
  toHex(): string {
    return (
      ...
      RgbColour.componentToHex(this.a)
    );
  }

  /**
   * Updates the properties of this instance to represent the same colour as the
   * provided string in the hexadecimal format #rrggbbaa.
   * @param hex A hexadecimal string in the format '#rrggbbaa'.
  ...
  loadFromHex(hex: string): RgbColour {
    ...
    this.a = hex.length > 7 ? parseInt(hex.slice(7, 9), 16) / 255 : 1;
    return this;
  }
...
export class HsvColour {
  ...
  /** The alpha of the colour, ranging from 0 to 1. */
  a: number;
...
export class FieldColourHsvSliders extends FieldColour {
  ...
  /** The maximum value of the alpha slider range. */
  private static readonly ALPHA_SLIDER_MAX = 100;
   ...
   * @returns The slider.
   */
  private static createSliderInContainer(max: number, step: number, container: HTMLElement): HTMLInputElement {
  ...
  private createDropdownSliders(): void {
    ...
    this.alphaReadout = FieldColourHsvSliders.createLabelInContainer("Alpha", container);
    this.alphaSlider = FieldColourHsvSliders.createSliderInContainer(
      FieldColourHsvSliders.ALPHA_SLIDER_MAX,
      1,
      container
    );
    ...
    this.hsvBoundEvents.push(
      Blockly.browserEvents.conditionalBind(this.saturationSlider, "input", this, this.onSliderChange)
    );
    this.hsvBoundEvents.push(
      Blockly.browserEvents.conditionalBind(this.brightnessSlider, "input", this, this.onSliderChange)
    );
    this.hsvBoundEvents.push(
      Blockly.browserEvents.conditionalBind(this.alphaSlider, "input", this, this.onSliderChange)
    );
   ...
   * @returns A hexadecimal representation of the colour in the format "#rrggbbaa"
   */
  private static hsvToHex(hue: number, saturation: number, brightness: number, alpha: number): string {
    ...
    FieldColourHsvSliders.helperHsv.a = alpha;
  ...
  private onEyedropperEvent(event?: Event): void {
    if (window.EyeDropper) {
      ...
      eyeDropper.open().then((result) => {
        this.setValue(result.sRGBHex + "FF");
        this.updateSliderValues();
      });
    }
  }
  ...
  private renderSliders(): void {
    ...
    this.alphaReadout.textContent = this.alphaSlider.value;

    const h: number = parseFloat(this.hueSlider.value) / FieldColourHsvSliders.HUE_SLIDER_MAX;
    const s: number = parseFloat(this.saturationSlider.value) / FieldColourHsvSliders.SATURATION_SLIDER_MAX;
    const v: number = parseFloat(this.brightnessSlider.value) / FieldColourHsvSliders.BRIGHTNESS_SLIDER_MAX;
    const a: number = parseFloat(this.alphaSlider.value) / FieldColourHsvSliders.ALPHA_SLIDER_MAX;

    // The hue slider needs intermediate gradient control points to include all
    // colours of the rainbow.
    let hueGradient = "linear-gradient(to right, ";
    hueGradient += FieldColourHsvSliders.hsvToHex(0 / 6, s, v, a) + ` ${FieldColourHsvSliders.THUMB_RADIUS}px, `;
    hueGradient += FieldColourHsvSliders.hsvToHex(1 / 6, s, v, a) + ", ";
    hueGradient += FieldColourHsvSliders.hsvToHex(2 / 6, s, v, a) + ", ";
    hueGradient += FieldColourHsvSliders.hsvToHex(3 / 6, s, v, a) + ", ";
    hueGradient += FieldColourHsvSliders.hsvToHex(4 / 6, s, v, a) + ", ";
    hueGradient += FieldColourHsvSliders.hsvToHex(5 / 6, s, v, a) + ", ";
    hueGradient +=
      FieldColourHsvSliders.hsvToHex(6 / 6, s, v, a) + ` calc(100% - ${FieldColourHsvSliders.THUMB_RADIUS}px))`;
    ...
    saturationGradient += FieldColourHsvSliders.hsvToHex(h, 0, v, a) + ` ${FieldColourHsvSliders.THUMB_RADIUS}px, `;
    saturationGradient +=
      FieldColourHsvSliders.hsvToHex(h, 1, v, a) + ` calc(100% - ${FieldColourHsvSliders.THUMB_RADIUS}px))`;
    ...
    brightnessGradient += FieldColourHsvSliders.hsvToHex(h, s, 0, a) + ` ${FieldColourHsvSliders.THUMB_RADIUS}px, `;
    brightnessGradient +=
      FieldColourHsvSliders.hsvToHex(h, s, 1, a) + ` calc(100% - ${FieldColourHsvSliders.THUMB_RADIUS}px))`;
    this.brightnessSlider.style.setProperty("--slider-track-background", brightnessGradient);

    // The alpha slider only needs gradient control points at each end.
    let alphaGradient = "linear-gradient(to right, ";
    alphaGradient += FieldColourHsvSliders.hsvToHex(h, s, v, 0) + ` ${FieldColourHsvSliders.THUMB_RADIUS}px, `;
    alphaGradient +=
      FieldColourHsvSliders.hsvToHex(h, s, v, 1) + ` calc(100% - ${FieldColourHsvSliders.THUMB_RADIUS}px))`;
    this.alphaSlider.style.setProperty("--slider-track-background", alphaGradient);
  }

  /** Updates slider values based on the current value of the field. */
  private updateSliderValues(): void {
    if (!this.hueSlider || !this.saturationSlider || !this.brightnessSlider || !this.alphaSlider) {
      return;
    }
    ...
    this.alphaSlider.value = String(hsv.a * FieldColourHsvSliders.ALPHA_SLIDER_MAX);
  ...
  protected doClassValidation_(newValue?: any): string | null {
    ...
    try {
      return new RgbColour().loadFromHex(newValue).toHex();
    } catch (e) {
      return null;
    }
  }
