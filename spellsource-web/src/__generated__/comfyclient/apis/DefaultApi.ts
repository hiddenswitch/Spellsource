...
export class DefaultApiRequestFactory extends BaseAPIRequestFactory {
  ...
  public async apiV1ImagesDigestGet(digest: string, _options?: Configuration): Promise<RequestContext> {
    ...
    // Path Params
    const localVarPath = "/api/v1/images/{digest}".replace("{" + "digest" + "}", encodeURIComponent(String(digest)));
  ...
  public async apiV1PromptsPost(
    ...
    // Body Params
    const contentType = ObjectSerializer.getPreferredMediaType(["application/json", "multipart/formdata"]);
  ...
  public async uploadImage(image?: HttpFile, _options?: Configuration): Promise<RequestContext> {
    ...
    if (!useForm) {
      const contentType = ObjectSerializer.getPreferredMediaType(["multipart/form-data"]);
      requestContext.setHeaderParam("Content-Type", contentType);
    }
...
export class DefaultApiResponseProcessor {
  ...
  public async apiV1PromptsPost(response: ResponseContext): Promise<ApiV1PromptsPost200Response | void> {
    ...
    if (isCodeInRange("500", response.httpStatusCode)) {
      throw new ApiException<undefined>(
        response.httpStatusCode,
        "An unexpected exception occurred and it is being passed to you.  This can occur if file was referenced in a LoadImage / LoadImageMask that doesn&#39;t exist. ",
        undefined,
        response.headers
      );
    }
    ...
    if (isCodeInRange("503", response.httpStatusCode)) {
      throw new ApiException<undefined>(
        response.httpStatusCode,
        "The server is too busy to process this request right now.  This should only be returned by a load balancer. Standalone comfyui does not return this. ",
        undefined,
        response.headers
      );
    }
  ...
  public async postPrompt(response: ResponseContext): Promise<string> {
    ...
    if (isCodeInRange("400", response.httpStatusCode)) {
      ...
      throw new ApiException<string>(
        response.httpStatusCode,
        "The prompt was invalid. The validation error is returned as the content.",
        body,
        response.headers
      );
    }
