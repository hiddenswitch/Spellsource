...
public class NaxxramasTests extends TestBase {
	...
	public void testAnubarAmbusher() {
		...
		runGym((context, player, opponent) -> {
			...
			assertTrue(ambusher.isDestroyed());
		...
		runGym((context, player, opponent) -> {
			...
			assertEquals(player.getHand().get(0).getCardId(), "minion_boulderfist_ogre");
		});
	}
	...
	public void testLoatheb() {
		runGym((context, player, opponent) -> {
			...
			assertEquals(costOf(context, player, friendlyFireball), friendlyFireball.getBaseManaCost());
			context.endTurn();
			Card fireball = receiveCard(context, opponent, "spell_fireball");
			assertEquals(costOf(context, player, friendlyFireball), friendlyFireball.getBaseManaCost());
			assertEquals(costOf(context, opponent, fireball), fireball.getBaseManaCost() + 5);
			context.endTurn();
			context.endTurn();
			assertEquals(costOf(context, opponent, fireball), fireball.getBaseManaCost());
		});
	}
	...
	public void testMadScientist() {
		runGym((context, player, opponent) -> {
			...
			assertEquals(opponent.getMinions().get(0).getSourceCard().getCardId(), "minion_stampeding_kodo");
		});
	...
	public void testPoisonSeedsAuchenai() {
		runGym((context, player, opponent) -> {
			...
			assertEquals(player.getHero().getHp(), GameLogic.MAX_HERO_HP - 10);
			Card poisonSeeds = CardCatalogue.getCardById("spell_poison_seeds");
			playCard(context, player, poisonSeeds);
			assertEquals(player.getHero().getHp(), GameLogic.MAX_HERO_HP);
		}, "BROWN", "WHITE");
	}
	...
	public void testPoisonSeedsHauntedCreeper() {
		runGym((context, player, opponent) -> {
			...
			for (Minion minion : player.getMinions()) {
				assertEquals(minion.getSourceCard().getCardId(), "token_spectral_spider");
			}
		}, "BROWN", "BLACK");
	}
}
