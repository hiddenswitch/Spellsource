...
public class GoblinsVersusGnomesTests extends TestBase {
	...
	public void testBlingtron3000() {
		runGym((context, player, opponent) -> {
			...
			assertEquals(player.getWeaponZone().size(), 1);
			assertEquals(opponent.getWeaponZone().size(), 1);
		});
	}
	...
	public void testMalganisVoidcallerVoidwalkerSheepInteraction() {
		...
		runGym((context, player, opponent) -> {
			...
			assertEquals(voidwalker.getAttack(), 3);
			assertEquals(voidwalker.getHp(), 1);
			Minion malganis = opponent.getMinions().get(1);
			assertEquals(malganis.getAttack(), 9);
			assertEquals(malganis.getHp(), 5);
	...
	public void testUnstablePortal() {
		runGym((context, player, opponent) -> {
			...
			assertEquals(costOf(context, player, yeti), yeti.getBaseManaCost() - 3);
		});
	}
	...
	public void testCallPet() {
		runGym((context, player, opponent) -> {
			...
			assertEquals(costOf(context, player, dred), dred.getBaseManaCost() - 4);
		});

		runGym((context, player, opponent) -> {
			Card yeti = shuffleToDeck(context, player, "minion_chillwind_yeti");
			playCard(context, player, "spell_call_pet");
			assertEquals(costOf(context, player, yeti), yeti.getBaseManaCost());
		});
	}
