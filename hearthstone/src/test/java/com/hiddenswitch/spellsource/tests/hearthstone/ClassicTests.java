...
public class ClassicTests extends TestBase {
	...
	public void testSouthseaDeckhand() {
		runGym((context, player, opponent) -> {
			...
			assertFalse(deckhand.canAttackThisTurn());
			playCard(context, player, "weapon_wicked_knife");
			assertTrue(deckhand.canAttackThisTurn());
	...
	public void testDivineShieldZeroAttackDefenderInteraction() {
		runGym((context, player, opponent) -> {
			...
			assertTrue(attacker.hasAttribute(Attribute.DIVINE_SHIELD));
		});
	}
	...
	public void testSpellbender() {
		...
		runGym((context, player, opponent) -> {
			...
			assertTrue(player.getGraveyard().stream().anyMatch(e -> e.getSourceCard().getCardId().equals("token_spellbender")));
		});
	}
	...
	public void testYsera() {
		...
		runGym((context, player, opponent) -> {
			...
			context.endTurn();
			assertTrue(yseraCards.anyMatch(c -> c.equals(player.getHand().get(0).getCardId())));
			assertEquals(player.getHand().size(), 1);
		});
	}
	...
	public void testShadowWordConditions() {
		runGym((context, player, opponent) -> {
			...
			player.setMaxMana(5);
			player.setMana(5);
			assertFalse(context.getValidActions().stream().anyMatch(ga -> ga instanceof PlaySpellCardAction
			...
			context.endTurn();
			assertTrue(context.getValidActions().stream().anyMatch(ga -> ga instanceof PlaySpellCardAction
					&& ((PlaySpellCardAction) ga).getSourceReference().equals(pain.getReference())));
			assertFalse(context.getValidActions().stream().anyMatch(ga -> ga instanceof PlaySpellCardAction
					&& ((PlaySpellCardAction) ga).getSourceReference().equals(death.getReference())));
			context.endTurn();
	...
	public void testAcidicSwampOoze() {
		runGym((context, player, opponent) -> {
			...
			assertNotNull(opponent.getHero().getWeapon());
			context.endTurn();

			playCard(context, player, "minion_acidic_swamp_ooze");
			assertNull(opponent.getHero().getWeapon());
		}, "BLUE", "RED");
	}
	...
	public void testSpitefulSmith() {
		runGym((context, player, opponent) -> {
			...
			assertTrue(player.getHero().getWeapon() != null);
	...
	public void testFaerieDragon() {
		runGym((context, player, opponent) -> {
			...
			assertFalse(validTargets.contains(elusiveOne));
			...
			assertNotEquals(faerieDragonHp, elusiveOne.getHp());
		}, "BLUE", "RED");
