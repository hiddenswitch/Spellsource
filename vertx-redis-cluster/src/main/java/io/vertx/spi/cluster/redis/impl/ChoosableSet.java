...
class ChoosableSet<T> implements ChoosableIterable<T> {
	...
	public ChoosableSet(Vertx vertx, Collection<? extends T> values) {
		...
		this.vertx = vertx;
		this.ids = Sets.newConcurrentHashSet(); /* new ConcurrentSkipListSet<T>(Comparator.<T, String>comparing(obj -> {
			if (obj instanceof ClusterNodeInfo) {
				return (((ClusterNodeInfo) obj).nodeId);
			} else {
				return obj.toString();
			}
		}));*/
