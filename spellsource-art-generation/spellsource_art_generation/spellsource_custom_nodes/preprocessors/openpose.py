...
class OpenPose_Preprocessor:
    ...
    def estimate_pose(self, image, detect_hand, detect_body, detect_face, **kwargs):
        from spellsource_preprocessors.controlnet_aux.open_pose import OpenposeDetector

        detect_hand = detect_hand == "enable"
        detect_body = detect_body == "enable"
        detect_face = detect_face == "enable"

        model = OpenposeDetector.from_pretrained(HF_MODEL_NAME, cache_dir=annotator_ckpts_path).to(
