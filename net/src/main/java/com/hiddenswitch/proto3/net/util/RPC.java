...
public class RPC {
	...
	/**
	 * Connects to the given {@code serviceInterface} on the {@link EventBus} and gives you an {@link NetworkedRpcClient} you
	...
	public static <T> RpcClient<T> connect(Class<? extends T> serviceInterface, final EventBus bus) {
		...
		NetworkedRpcClient<T> result = new NetworkedRpcClient<>((T) Proxy.newProxyInstance(
