...
def retrieve_matchups_rows():
    ...
    _archetypes_request = urllib.Request(url='https://hsreplay.net/api/v1/archetypes/', headers=_headers)
    _archetypes_list = json.loads(urllib.urlopen(_archetypes_request).read())
    _data_request = urllib.Request(
    ...
    _matchups_dict = json.loads(urllib.urlopen(_data_request).read())
